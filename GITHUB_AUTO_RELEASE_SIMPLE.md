...
5. Faire `git push` sur `main` ou `master` et laisser GitHub Actions générer les installateurs Windows/Linux.   

Le ZIP source propre reste utile pour une reprise de développement ou un envoi manuel, mais il ne doit pas être exposé comme asset principal de release joueur.   
...
- compile les paquets de jeu Linux/Windows nécessaires au fonctionnement des installateurs ;   
- place ces paquets comme secours local dans les packs installateurs ;   
- publie ou répare la release GitHub ;   
- vérifie que les deux ZIP installateurs Windows/Linux sont bien présents avant de considérer la release complète.   

Si une release existe mais ne contient pas les bons installateurs, elle est traitée comme incomplète et le workflow republie les assets nécessaires.   
